...
bool OverlayWindow::CreateGraphicsResources() {
    m_graphics = std::make_unique<OverlayGraphics>(m_hWnd);
...
bool OverlayWindow::CreateDeviceComponents() {
    ...
    if (SUCCEEDED(status)) {
        status = LoadBitmapFromFile(m_graphics->GetContext2D(), m_wicFactory.Get(), L"1.webp", 342, 165, m_d2dMapBitmap.GetAddressOf());
    }
...
void OverlayWindow::Render() {
    ...
    m_graphics->GetContext2D()->CreateGradientStopCollection(overlayBaseGradientStops, 3, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, overlayBaseGradientStop.GetAddressOf());
    ...
    m_graphics->GetContext2D()->CreateGradientStopCollection(overlayStrokeGradientStops, 2, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, overlayStrokeGradientStop.GetAddressOf());
    ...
    m_graphics->GetContext2D()->CreateGradientStopCollection(mapStrokeGradientStops, 2, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, mapStrokeGradientStop.GetAddressOf());
    ...
    m_graphics->GetContext2D()->CreateGradientStopCollection(mapframeGradientStops, 5, D2D1_GAMMA_2_2, D2D1_EXTEND_MODE_CLAMP, mapframeGradientStop.GetAddressOf());

    // Brushes
    ComPtr<ID2D1RadialGradientBrush> overlayBaseBrush;
    m_graphics->GetContext2D()->CreateRadialGradientBrush(
    ...
    m_graphics->GetContext2D()->CreateSolidColorBrush(D2D1::ColorF(0x5F5F5F), mapStrokeBrush.GetAddressOf());

    ComPtr<ID2D1LinearGradientBrush> mapFrameBrush;
    m_graphics->GetContext2D()->CreateLinearGradientBrush(
    ...
    CreateFillRoundedRectangleGeometryEx(m_graphics->GetFactory2D(),
    ...
    CreateFrameGeometry(m_graphics->GetFactory2D(), Frame(MapFrameRect, D2D1::Point2F(90.0f, 30.0f), D2D1::Point2F(MapFrameStroke * 4, MapFrameStroke * 4)), frameGeometry.GetAddressOf());

    // Overlay rendering preparation
    ComPtr<ID2D1BitmapRenderTarget> overlayRenderTarget;
    m_graphics->GetContext2D()->CreateCompatibleRenderTarget(overlayRenderTarget.GetAddressOf());
    ...
    m_graphics->GetContext2D()->BeginDraw();
    m_graphics->GetContext2D()->Clear(D2D1::ColorF(0x000000, 0.0f));

    // Overlay shadow
    ComPtr<ID2D1Effect> shadowEffect;
    m_graphics->GetContext2D()->CreateEffect(CLSID_D2D1Shadow, shadowEffect.GetAddressOf());
    ...
    m_graphics->GetContext2D()->CreateEffect(CLSID_D2D12DAffineTransform, &affineTransformEffect);
    ...
    m_graphics->GetContext2D()->DrawImage(affineTransformEffect.Get());
    m_graphics->GetContext2D()->DrawImage(overlayBitmap.Get());

    // Glare rendering
    // m_graphics->GetContext2D()->SetTransform(D2D1::Matrix3x2F::Rotation(0.0f) * D2D1::Matrix3x2F::Translation(0.0f, 0.0f));
    // m_graphics->GetContext2D()->SetTransform(D2D1::Matrix3x2F::Identity());

    // End
    status = m_graphics->GetContext2D()->EndDraw();
    status = m_graphics->GetDxgiSwapChain()->Present(1, 0);
}
